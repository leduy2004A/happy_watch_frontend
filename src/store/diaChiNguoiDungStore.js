...
import { layToanBoDiaChiCuaKhachHangToken } from "@/axios/khachhang";
import { getAllDiaChiKhachHang } from "@/axios/khachhang";
import { themDiaChiKhachHang } from "@/axios/khachhang";
import { themDiaChiKhachHangToken } from "@/axios/khachhang";
import { useAddressStore } from "./diaChiStore";
import { useToast } from "vue-toastification";
import { useOrderStore } from "./tienStore";
const toast = useToast()
export const useDiaChiNguoiDungStore = defineStore("DiaChiNguoiDungStore", {
  state: () => ({
    idNguoiDung:0,
    data: [
      {
  ...
  actions: {
    async fetchDataDiaChiNguoiDung(){
        const result = await layToanBoDiaChiCuaKhachHangToken()
    ...
    console.log(addressStore.formData.province)
      try {
        const result = await themDiaChiKhachHangToken(dataAdress)
        if (result.status === 200) {
