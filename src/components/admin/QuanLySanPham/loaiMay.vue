...
<template>
    <BaseCrud title="Quản lý loại máy" :items="data" :table-headers="headers">
  ...
  import { layTatCaLoaiMay,addLoaiMay,updateLoaiMay } from "@/axios/sanpham";
  // import { addThuongHieu } from "@/axios/sanpham";
  const emitter = useEmitter();
  // Table headers
  const headers = [
    // { title: "STT", key: "stt", align: "center" },
    { title: "Tên Loại Máy", key: "ten", align: "center" },
  ...
  const handleCreate = async (newItem) => {
    ...
    try {
      ...
      const result = await addLoaiMay(newItem);
      if (result.status === 200) {
        toast.success("Thêm hình dáng thành công");
  ...
  const handleUpdate = async ({ item, index }) => {
    try {
      ...
      const result = await updateLoaiMay(item)
      if(result.status === 200)
      {
        data.value[index] = { ...item, id: data.value[index].id };
        toast.success("Sửa hình dáng thành công")
      }else {
        toast.error("Có lỗi xảy ra");
      }
    } catch (error) {
  ...
  const handleDelete = async (item) => {
    try {
      ...
      const index = data.value.findIndex((user) => user.id === item.id);
  ...
  onMounted(async () => {
    ...
    try {
      const result = await layTatCaLoaiMay();
