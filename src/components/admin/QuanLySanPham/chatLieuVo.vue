...
<template>
    <BaseCrud title="Quản lý chất liệu vỏ" :items="data" :table-headers="headers">
  ...
  import { layTatCaChatLieuVo,addChatLieuVo,updateChatLieuVo } from "@/axios/sanpham";
  // import { addThuongHieu } from "@/axios/sanpham";
  const emitter = useEmitter();
  // Table headers
  const headers = [
    // { title: "STT", key: "stt", align: "center" },
    { title: "Tên Chất Liệu Vỏ", key: "ten", align: "center" },
  ...
  const handleCreate = async (newItem) => {
    ...
    try {
      ...
      const result = await addChatLieuVo(newItem);
      if (result.status === 200) {
        toast.success("Thêm chất liệu vỏ thành công");
  ...
  const handleUpdate = async ({ item, index }) => {
    try {
      ...
      const result = await updateChatLieuVo(item)
      if(result.status === 200)
      {
  ...
  onMounted(async () => {
    ...
    try {
      const result = await layTatCaChatLieuVo();
