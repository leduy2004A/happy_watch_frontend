...
<template>
    <BaseCrud title="Quản lý màu sắc" :items="data" :table-headers="headers">
  ...
  import { layTatCaMauSac,addMauSac,updateMauSac } from "@/axios/sanpham";
  // import { addThuongHieu } from "@/axios/sanpham";
  const emitter = useEmitter();
  // Table headers
  const headers = [
    // { title: "STT", key: "stt", align: "center" },
    { title: "Tên Màu Sắc", key: "ten", align: "center" },
  ...
  const handleCreate = async (newItem) => {
    ...
    try {
      ...
      const result = await addMauSac(newItem);
  ...
  const handleUpdate = async ({ item, index }) => {
    try {
      ...
      const result = await updateMauSac(item)
      if(result.status === 200)
      {
  ...
  onMounted(async () => {
    ...
    try {
      const result = await layTatCaMauSac();
