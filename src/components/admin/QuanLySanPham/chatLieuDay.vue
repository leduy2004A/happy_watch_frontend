...
<template>
    <BaseCrud title="Quản lý chất liệu dây" :items="data" :table-headers="headers">
  ...
  import { layTatCaChatLieuDay,addChatLieuDay,updateChatLieuDay } from "@/axios/sanpham";
  // import { addThuongHieu } from "@/axios/sanpham";
  const emitter = useEmitter();
  // Table headers
  const headers = [
    // { title: "STT", key: "stt", align: "center" },
    { title: "Tên Chất Liệu Dây", key: "ten", align: "center" },
  ...
  const handleCreate = async (newItem) => {
    ...
    try {
      ...
      const result = await addChatLieuDay(newItem);
      if (result.status === 200) {
        toast.success("Thêm chất liệu dây thành công");
  ...
  const handleUpdate = async ({ item, index }) => {
    try {
      ...
      const result = await updateChatLieuDay(item)
      if(result.status === 200)
      {
        data.value[index] = { ...item, id: data.value[index].id };
        toast.success("Sửa chất liệu dây thành công")
      }else {
        toast.error("Có lỗi xảy ra");
      }
    } catch (error) {
  ...
  onMounted(async () => {
    ...
    try {
      const result = await layTatCaChatLieuDay();
