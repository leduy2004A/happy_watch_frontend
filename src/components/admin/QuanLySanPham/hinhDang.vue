...
<template>
  ...
  import { layTatCaHinhDang,addHinhDang,updateHinhDang } from "@/axios/sanpham";
  // import { addThuongHieu } from "@/axios/sanpham";
  const emitter = useEmitter();
  // Table headers
  const headers = [
    // { title: "STT", key: "stt", align: "center" },
    { title: "Tên Hình Dáng", key: "ten", align: "center" },
  ...
  const handleCreate = async (newItem) => {
    ...
    try {
      ...
      const result = await addHinhDang(newItem);
  ...
  const handleUpdate = async ({ item, index }) => {
    try {
      ...
      const result = await updateHinhDang(item)
      if(result.status === 200)
      {
  ...
  onMounted(async () => {
    ...
    try {
      const result = await layTatCaHinhDang();
