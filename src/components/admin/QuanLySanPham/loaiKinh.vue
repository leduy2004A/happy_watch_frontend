...
<template>
    <BaseCrud title="Quản lý loại kính" :items="data" :table-headers="headers">
  ...
  import { layTatCaLoaiKinh,addLoaiKinh,updateLoaiKinh } from "@/axios/sanpham";
  ...
  const handleCreate = async (newItem) => {
    ...
    try {
      ...
      const result = await addLoaiKinh(newItem);
      if (result.status === 200) {
        toast.success("Thêm loại kính thành công");
  ...
  const handleUpdate = async ({ item, index }) => {
    try {
      ...
      const result = await updateLoaiKinh(item)
      if(result.status === 200)
      {
        data.value[index] = { ...item, id: data.value[index].id };
        toast.success("Sửa loại kính thành công")
      }else {
        toast.error("Có lỗi xảy ra");
      }
    } catch (error) {
  ...
  onMounted(async () => {
    ...
    try {
      const result = await layTatCaLoaiKinh();
