      ...
      <Column style="width: 50px">
        ...
        <template #body="slotProps">
          <Checkbox 
            ...
            @update:modelValue="(value) => store.toggleProductSelection(slotProps.data.chiTietSanPhamId, value)"
            :binary="true"
          />
        </template>
      </Column>
      ...
      <Column field="tenSanPham" header="Sản phẩm">
        <template #body="slotProps">
          <div class="flex align-items-center">
            <Image :src="slotProps.data.hinhAnh" width="50" height="50" class="mr-2" />
            <span>{{ slotProps.data.tenSanPham }}</span>
          </div>
        </template>
      </Column>
        ...
        <template #body="slotProps">
          <div class="flex align-items-center justify-content-center">
            <Button 
              ...
              text
            />
            <span class="mx-2">{{ slotProps.data.quantity }}/{{ slotProps.data.soLuong }}</span>
            <Button 
              icon="pi pi-plus"
            ...
            <Column header="Ghi chú">
              <template #body="slotProps">
                <InputText 
                  ...
                  @input="(e) => store.updateNote(slotProps.data.chiTietSanPhamId, e.target.value)"
                />
              </template>
            </Column>
          </DataTable>
          ...
          <div class="mb-4">
            ...
            <div class="flex align-items-start" v-if="store.diaChi.diaChiCuThe && store.diaChi.xaPhuongThiTran && store.diaChi.quanHuyen && store.diaChi.tinhThanhPho != ''">
              ...
              <div>
                ...
                <div>{{ store.diaChi.diaChiCuThe }}, {{ store.diaChi.xaPhuongThiTran }}, {{ store.diaChi.quanHuyen }}, {{ store.diaChi.tinhThanhPho }}</div>
              </div>
            </div>
          </div>
