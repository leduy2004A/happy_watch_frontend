<template>
  ...
  const confirmPayment = async () => {
    const dataPayment = {
      ...
      soTien:customerPayment.value,
      phuongThuc:paymentMethod.value
    };
    console.log(dataPayment);
    try{
      const resultPayment =  await thanhToanHoaDon(dataPayment);
      if(resultPayment.status == 200)
      {
        toast.success("Thanh toán thành công")
        transactions.value.push(resultPayment.data.thanhToan);
        paymentStore.fetchPaymentHistory(route.params.ma)
        timeLineStore.fetchTimeline(route.params.ma)
      }
