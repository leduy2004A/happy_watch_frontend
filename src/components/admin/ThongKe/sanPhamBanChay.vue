  ...
  <div class="card">
    ...
    <div class="filter-section mb-4">
      <div class="p-buttonset">
        <Button label="NGÀY" @click="setFilterType('day')" :class="{ 'p-button-outlined': filterType !== 'day' }" />
        <Button label="TUẦN" @click="setFilterType('week')" :class="{ 'p-button-outlined': filterType !== 'week' }" />
        <Button label="THÁNG" @click="setFilterType('month')" :class="{ 'p-button-outlined': filterType !== 'month' }" />
        <Button label="NĂM" @click="setFilterType('year')" :class="{ 'p-button-outlined': filterType !== 'year' }" />
        <Button label="TÙY CHỈNH" @click="setFilterType('custom')" :class="{ 'p-button-outlined': filterType !== 'custom' }" />
      </div>
      ...
      <div class="date-filters mt-3" v-if="filterType === 'custom'">
        ...
        <Button label="EXPORT TO EXCEL" icon="pi pi-file-excel" class="p-button-success ml-2" @click="exportToExcel" />
      </div>
    </div>
<div class="ban-chay">
  <div class="">
  ...
  <DataTable :value="products" :paginator="true" :rows="5" responsive-layout="scroll"
    ...
    <div class="chart-section mt-4 chart">
      ...
      <Chart type="pie" :data="chartData" :options="chartOptions" style="width: 400px" />
    </div>
</div>
    <!-- Products Table -->
...
import { getThongKeHomNaySanPham,getThongKeThangNaySanPham,getThongKeTuanNaySanPham,getThongKeTheoNamSanPham } from '@/axios/thongKe';
...
// Dữ liệu biểu đồ
const chartData = {
  labels: ['Đã hủy', 'Chờ xác nhận', 'Chờ giao hàng', 'Đang vận chuyển',
          'Đã giao hàng', 'Đã thanh toán', 'Hoàn thành', 'Trả hàng'],
  datasets: [
    {
      data: [7.14, 14.29, 14.29, 0, 0, 7.14, 42.86, 14.29],
      backgroundColor: [
        '#FF6384', '#FF9F40', '#FFCD56', '#4BC0C0',
        '#36A2EB', '#9966FF', '#00CC66', '#C9CBCF',
      ],
    },
  ],
};
